#!/bin/bash
# Связывает два эмулятора как пиров: redir портов + DEBUG_PEER. Нужно на Mac: mDNS между двумя AVD на macOS не ходит вовсе
# (docs/refactor-plan.md, A4). Порты приложения меняются при каждом запуске — скрипт можно (и нужно) запускать заново после
# перезапуска приложения.
source "$(dirname "$0")/lib.sh"
# E2E_STATIC_PEERS=0 (ручной запуск e2e.yml со снятым static_peers) — без статических пиров, только NSD. Ждём, что каждый нашёл другого
