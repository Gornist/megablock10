#!/bin/bash
# Связывает два эмулятора как пиров: redir портов + DEBUG_PEER. mDNS между эмуляторами ходит, но одного NSD стенду пока мало
# (docs/refactor-plan.md, A4: после перезапуска приложения NSD отдаёт порт прошлого процесса).
