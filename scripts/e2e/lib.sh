...
# link.sh не мог связать эмуляторы. Теперь спрашиваем у самого приложения (DEBUG_CONFIG port). Запасной путь по старой строке
# убран: приложение её больше не пишет (всплыло при составлении scripts/e2e/log-contract.tsv).
port_of() {
  local p
  ...
  # Порт не больше 1024 (в т.ч. -1) — сервер ещё не слушает: пусто, и link.sh ждёт дальше.
  [ -n "$p" ] && [ "$p" -gt 1024 ] 2>/dev/null && echo "$p"
}
