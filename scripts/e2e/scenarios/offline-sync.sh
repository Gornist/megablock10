...
server_stop
adb_ $A logcat -c
dbg $A DEBUG_SET --es pay "$PKB:3:offline"
TX=$(await 15 tx_of $A); [ -n "$TX" ] || die "перевод не создан"
check "очередь на телефоне не пуста, пока сервера нет" wait_until 15 bash -c "source '$ROOT/scripts/e2e/lib.sh'; [ \"\$(q $A 'select count(*) from pending_change_records')\" -gt 0 ]"
