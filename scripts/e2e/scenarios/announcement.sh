...
for s in $DEVICES; do
  ...
  start_app $s
  check "$s: окно «Сообщение от мастера» на экране" wait_until 20 screen_has $s "Сообщение от мастера"
  tap_text $s "Принято" >/dev/null; sleep 1
  check "$s: после «Принято» окна нет" bash -c "source '$ROOT/scripts/e2e/lib.sh'; ! screen_has $s 'Сообщение от мастера'"
done
