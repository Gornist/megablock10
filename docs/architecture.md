...
снимки состояния в журнал). Сессию поднимает `AppGraph.startMeshWhenIdentityAppears()` при появлении персонажа — из
`processScope`, с момента запуска процесса, не из экрана: иначе после перезапуска процесса системой (без Activity)
приём сообщений не работал бы, пока игрок сам не откроет приложение. Один раз на ключ, правки мастера её не
перезапускают (`identity/MeshAutostart.kt`, `startMeshOncePerCharacter`). Останавливает сессию `SessionReset`.
