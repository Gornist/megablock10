...
Android 8.0. В приложении то же сторожит Android Lint (`:app:lintDebug`, проверка `NewApi`).
...
снимки состояния в журнал). Сессию поднимает `SessionViewModel` при появлении персонажа — один раз на ключ, правки
мастера её не перезапускают. Останавливает сессию `SessionReset`. Приём чека, пришедшего по сети, сразу подтверждает
